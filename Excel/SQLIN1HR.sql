...
-- CREATE A BAND TABLE WITH ID AND BAND NAME
...
-- TO INPUT ID NUMBER EXPLICITLY YOU ON IDENTITY INSERT AFTERWARDS YOU OFF
...
SET IDENTITY_INSERT bands OFF -- it is important to turn it off incase you want to input more info without explicitly inputting the id 


SELECT * FROM bands

-- CREATE A ALBUM TABLE WITH ID, NAME OF ALBUM, RELEASE YEAR AND ITS BAND ID CONNECTING IT USING FORIEGN KEY
...
EXEC sp_rename 'record_company.dbo.albums.release_date', 'release_year', 'COLUMN';


-- EXEC sp_rename 'old_table_name','new_table_name';  FOR CHANGING OF TABLE NAME.
...
-- CREATE A SONG TABLE WITH ID, NAME OF SONG, LENGTH OF SONG AND ITS ALBUMS CONNECTING USING FK WITH ALBUM ID
...
ALTER TABLE songs ALTER COLUMN length 
...
-- RENAME BAND NAME TO BAND NAME

SELECT name AS Band_Name FROM bands

-- SELECT THE OLDEST ALBUM

SELECT TOP 1 id, name, release_year, band_id FROM albums
WHERE release_year IS NOT NULL
ORDER BY release_year;

--GET ALL BAND THAT HAVE ALBUMS

SELECT DISTINCT bands.name AS Band_Name FROM bands 
INNER JOIN albums ON bands.id = albums.band_id;

--GET ALL BAND THAT HAVE NO ALBUMS
...
SELECT TOP 1 albums.name AS Name, albums.release_year AS Release_Year, SUM(songs.length) AS Duration FROM albums
...
INSERT INTO bands(id, name) VALUES(8, 'Burna Boy')
INSERT INTO albums(id,name, release_year, band_id) VALUES (19,'Love Damini',2022, 8)
...
-- DELETE THE BAND AND ALBUM THAT YOU ADDED IN PREVIOUS TASK (MIND THE ORDER OF REMOVAL BCUZ OF THE FK AND PK)

DELETE FROM albums 
WHERE name = 'Love Damini'; 
...
SELECT AVG(songs.length) AS Average_Song_Duration FROM songs

--SELECT THE LONGEST SONG OFF EACH ALBUM

SELECT TOP 18 albums.name AS Album,
albums.release_year AS Release_Year, MAX(songs.length) AS Duration FROM albums
JOIN songs ON albums.id = songs.album_id
GROUP BY albums.name, albums.release_year, songs.length
ORDER BY Duration DESC;

--GET THE NUMBER OF SONGS FOR EACH BAND
--note -- not complete

SELECT bands.name AS Band, COUNT(songs.name) AS Number_of_songs FROM bands
INNER JOIN albums ON bands.id = albums.band_id
INNER JOIN songs ON albums.id = songs.album_id
GROUP BY bands.name, songs.name;
